...
int main(){
    ...
    for(int i = 0; i <= SimulationCompletionTime; i = i + ProcessCompletionTime ){ 
        ...
        ProcessType = rand() % ProcessCount + 1; //süreç tipi 1 ve toplam süreç sayısı arasında rastgele seçilir.
        cout << "Process completion time: " << ProcessCompletionTime << endl;
        cout << "Process Type: " << ProcessType << " | Time " << i << "s: event happened." << endl;      
    }
}
